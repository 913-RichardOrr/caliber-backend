...
export async function handler(event: CombinedEvent) {
  ...
  if (event.queryStringParameters.year) {
    let batchInfoResponse = await getAllBatchesLambda(event.queryStringParameters.year);
	if (batchInfoResponse) {
		...
		if (bodyTrainer) {
			...
			let validYearsArray = batchInfo.map((batch) => {return new Date(batch.startDate).getFullYear()})
			let validYearsSet = new Set(validYearsArray)
			validYears = JSON.stringify(Array.from(validYearsSet))
			return createResponse(JSON.stringify({validYears: JSON.parse(validYears), batches: batchInfo}))
		} else {
			return createResponse('Not Found', 404)
		}
